...
from casatasks import delmod, uvsub, split
from casatools import table
import gc, traceback
from optparse import OptionParser


def add_column(msname, colnames):
    """
    ...
    -------
    bool
        Column insertation is successful or not
    """
    from pyrap.tables import table as pytable
    success = False
    try:
        print("Opening {}...".format(msname))
        tb = pytable(msname, readonly=False)
    ...
    for colname in colnames:
        if colname not in tb.colnames():
            ...
            print("Adding column '{}' to {}...".format(colname, msname))
            tb.addcols(desc, dminfo)
            success = True
    print("Columns added to {} successfully.".format(msname))
    tb.close()
    return success
...
def peel_source(
    ...
    if msg == 0:
        msg = add_column(msname, ["MODEL_DATA_" + source_name])
        if msg:
            tb = table()
            tb.open(msname, nomodify=False)
            model_data = tb.getcol("MODEL_DATA")
            tb.putcol("MODEL_DATA_" + source_name, model_data)
            tb.flush()
            tb.close()
            model_col_name = "MODEL_DATA_" + source_name
            ...
            os.system("rm -rf chgcentre.log")
            gc.collect()
            return 0, model_col_name
        else:
            print("Error in adding model column")
...
def run_peel(
    ...
    try:
        ...
        tb.close()
        outputms = (
            basedir + "/" + os.path.basename(msname).split(".ms")[0] + "_peeled.ms"
        )
        if (
            os.path.exists(outputms)
            and os.path.exists(outputms + "/.peeled")
            and force_peel == False
        ):
        ...
        if len(peel_source_dic) == 0:
            ...
            model_col_list = []
            if os.path.exists('ds9_reg.reg'):
                os.system('rm -rf ds9_reg.reg')
            append=False
            for source in source_names:
                source_ra, source_dec, alt, az, appflux = peel_source_dic[source]
                reg_file=create_ds9_region(source_ra, source_dec, 1800, append=append, reg_file="ds9_reg.reg")
                append=True
                success, source_model_col_name = peel_source(
                    msname,
                    metafits,
                    peeldir,
                    source,
                ...
                if success == 0 and source_model_col_name != None:
                    model_col_list.append(source_model_col_name)
            if len(model_col_list) == 0:
                print("No source models to peel.")
                os.system("touch " + msname + "/.peeled")
                return 0, msname
            delmod(vis=msname, otf=False, scr=True)
            fits_mask=make_fits_mask_from_regions(ref_fits_file,reg_file,reg_file_format="ds9",output_mask="central_field_mask.fits",invert_mask=True)
            
            msg, imagedir, imageprefix = perform_spectrotemporal_imaging(
                msname,
                freqres=1280,
                ntime=1,
                imagedir=peeldir + "/central_fov",
                use_multiscale=use_multiscale,
                multiscale_scales=multiscale_scales,
                weight=weight,
                robust=robust,
            ...
            model_col_list.insert("MODEL_DATA", 0)
            model_col = ":".join(model_col_list)
            qc_args = []
            solver_terms = []
            num_iter = []
            subtract_directions = []
            all_dirc = list(range(len(model_col)))
            for i in range(len(model_col)):
                solver_terms.append(f"J{i}")
                if i == 0:
                    num_iter.append(100)
                    qc_args.append(f"J{i}.direction_dependent=False")
                else:
                    num_iter.append(50)
                    subtract_directions.append(i)
                    qc_args.append(f"J{i}.direction_dependent=True")
                qc_args.append(f"J{i}.type=complex")
                qc_args.append(f"J{i}.freq_interval={freq_interval}")
                qc_args.append(f"J{i}.time_interval={time_interval}")
                pinned_directions = copy.deepcopy(all_dirc).remove(i)
                qc_args.append(f"J{i}.pinned_directions={pinned_directions}")
            qc_args.append(f"")
            quartical_cal_peel_cmd = (
                f"goquartical input_ms.path={msname} input_ms.data_column=DATA input_ms.weight_column=WEIGHT_SPECTRUM input_ms.time_chunk={time_interval} input_ms.freq_chunk={freq_interval} input_model.recipe={model_col} input_model.apply_p_jones=False output.gain_directory={caltable} output.log_directory={qc_log} output.log_to_terminal={verbose} output.overwrite=True output.products=[corrected_residual] output.columns=[CORRECTED_RESIDUAL] output.flags=True output.apply_p_jones_inv=False output.subtract_directions={subtract_directions} solver.terms={solver_terms} solver.iter_recipe={num_iter} solver.threads=4 solver.convergence_fraction=0.99 solver.convergence_criteria=1e-06 solver.reference_antenna=0 dask.threads=8 dask.workers=1 "
                + " ",
                join(qc_args),
            )
            if verbose:
                print(quartical_cal_peel_cmd)
            os.system(quartical_cal_peel_cmd)
            if (
                modify_final_datacolumn
            ):  # Modify datacolumn with the self-calibrated data
                tb.open(msname, nomodify=False)
