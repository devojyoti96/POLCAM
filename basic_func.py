...
def get_free_cpu_and_mem():
    """
    Get numbers of free CPU threads and memory in GB
    Returns
    -------
    int
        Number of free CPU threads
    float
        Free memory in GB
    """
    current_cpu_usage = psutil.cpu_percent()
    available_cpu = int(psutil.cpu_count() * (1 - (current_cpu_usage / 100.0)))
...
def get_ateam_sources(
    ...
    pos["PKS2356"] = ["23h59m04.36s", "-60d54m59.41s", 123.8, -0.75, 145]
    pos["PKS2153"] = ["21h57m05.98s", "-69d41m23.68", 131.4, -0.56, 145]
    pos["PKS0408"] = ["04h08m20.0s", "-65d45m08.0s", 5.9, -0.85, 150]
    pos["PKS0410"] = ["04h10m48.0s", "-75d07m19.0s", 5.0, -0.85, 150]
    pos["3C33"] = ["01h08m52.85s", "+13d20m13.75s", 54.2, -0.33, 150]
    pos["3C161"] = ["06h27m10.09s", "-05d53m04.76s", 74.9, -0.37, 160]
    pos["PKS0442"] = ["04h44m37.70s", "-28d09m54.41s", 37.0, -0.85, 150]
    pos["3C409"] = ["20h14m27.6s", "+23d34m53s", 99.0, -0.57, 160]
    pos["PKS0351"] = ["03h51m35.7s", "-27d44m35s", 52.0, -1.09, 80]
...
def create_ds9_region(ra, dec, radius, append=False,  reg_file="ds9_reg.reg"):
    """
    Function to create a DS9 region file
    ra : float, str
        RA of the center of the region (formats: degree, h:m:s d:m:s, --h--m--s --d--m--s)
    dec : float
        DEC of the center of the region (formats: degree, h:m:s d:m:s, --h--m--s --d--m--s)
    radius : float
        Circular region radius in arcseconds
    append : bool, optional
        Append with existing region file 
    ...
    """
    ds9_header = f"""# Region file format: DS9 CARTA 3.0.0-beta.3 
...
fk5"""
    ds9_region=f"""circle({ra}, {dec}, {radius}") # color=green """
...
def make_fits_mask_from_regions(
    ref_fits_file,
    reg_file,
    reg_file_format="ds9",
    ...
    """
    Make fits mask from a region file (in ds9, crtf or fits format) and given reference fits file
    Parameters
    ----------
    ref_fits_file : str
        Reference fits file
    reg_file : str
        Region file
    reg_file_format : str, optional
        Region file format (ds9, crtf, fits)
    output_mask : str, optional
        Output mask fits name
    ...
    str
        Fits mask file
    """
    # Load WCS from reference FITS file
    with fits.open(ref_fits_file) as hdul:
        header = hdul[0].header
        wcs = WCS(header)
        wcs = wcs.celestial  # Extract only RA/Dec (ignores extra axes)
        # Modify the header to reflect 2D WCS
        new_header = wcs.to_header()
        image_shape = hdul[0].data.shape[-2:]  # Get only spatial dimensions
    # Load DS9 region
    ...
    for region in regions:
        if isinstance(region, CircleSkyRegion):
            ...
            x, y = int(pixel_region.center.x), int(pixel_region.center.y)
            r = int(pixel_region.radius)  # FIXED: No need for .to('pixel').value
        else:
            print(f"Skipping unsupported region: {region}")
            continue
        # Rasterize the circle into the mask
        y_grid, x_grid = np.ogrid[:image_shape[0], :image_shape[1]]
        mask_region = (x_grid - x) ** 2 + (y_grid - y) ** 2 <= r ** 2
        mask[mask_region] = 1
    if invert_mask:
        pos0=np.where(mask==0)
        pos1=np.where(mask==1)
        mask[pos0]=1
        mask[pos1]=0
    # Save mask as a FITS file
    if "BUNIT" in new_header:
        del new_header["BUNIT"]
    if "BMAJ" in new_header:
        del new_header["BMAJ"]
    if "BMIN" in new_header:
        del new_header["BMIN"]
    if "BPA" in new_header:
        del new_header["BPA"]
    if "SPECSYS" in new_header:
        del new_header["SPECSYS"]    
    hdu = fits.PrimaryHDU(mask.astype(np.uint8), header=new_header)
