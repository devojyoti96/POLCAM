...
def make_caltable_columns(msname,caltable,nchan):
    ...
    coldes = {'comment': '','dataManagerGroup': 'MSMTAB',
    ...
    coldes_time = {'comment': '',
             ...
             'option': 5,
             'valueType': 'double'}  
    coldes_interval={'comment': '',
         'dataManagerGroup': 'MSMTAB',
         'dataManagerType': 'StandardStMan',
         'keywords': {'QuantumUnits': np.array(['s'], 
                dtype='|S2')},
         'maxlen': 0,
         'option': 5,
         'valueType': 'double'}
    # Define the table description for the calibration table
    flag=makearrcoldesc('FLAG', np.zeros((nchan,2), dtype=bool))  # Boolean array column
    ...
    for spw in range(num_spws):
        for ant in range(num_ants):
            ...
            # Fill array columns
            bandpass_data = np.ones((nchan, 2), dtype=np.complex64)  # Normalized bandpass
...
def create_blank_caltable(msname,caltable,nchan):
    ...
    process1 = Process(target=create_blank_table, args=(msname,caltable,))
    process1.start()
    process1.join() 
    process2 = Process(target=make_caltable_columns, args=(msname,caltable,nchan,))
    process2.start()
    process2.join() 
